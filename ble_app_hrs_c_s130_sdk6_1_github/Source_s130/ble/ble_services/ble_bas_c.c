...
static void on_peer_central_write(ble_bas_c_t * p_ble_bas_c, const ble_evt_t * p_ble_evt)
{
    ...
    if( p_ble_bas_c->conn_handle != BLE_CONN_HANDLE_INVALID )
    {
        ...
        p_write_params->flags    = BLE_GATT_EXEC_WRITE_FLAG_PREPARED_WRITE;
...
void ble_bas_c_on_ble_evt(ble_bas_c_t * p_ble_bas_c, const ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            if(p_ble_evt->evt.gap_evt.params.connected.role == BLE_GAP_ROLE_CENTRAL) //If this side is BLE_GAP_ROLE_CENTRAL then BLE_GAP_EVT_CONNECTED comes from peer peropheral
            {
